...
if ( $DEBUG == "yes" )
{
    Write-Output "Create Host via REST-API"
}
$headers = @{
...
# Accept all found labels for the newly created host via REST-API
if ( $DEBUG == "yes" )
{
    Write-Output "Accept all found labels for the newly created host via REST-API"
}
$headers = @{
    'Accept' = 'application/json'
    'Authorization' = "Bearer $USERNAME $PASSWORD"
